...
async function fetchPrices(mints: string[]): Promise<Record<string, PriceRow>> {
  if (!mints.length) return {};
  try {
    const base = process.env.PRICES_BASE_URL || process.env.WEB_BASE_URL || '';
    if (!base) return {};
    const qs = mints.map((m) => `mints=${encodeURIComponent(m)}`).join('&');
  const resp = await fetch(`${base.replace(/\/$/, '')}/api/prices?${qs}`);
...
async function computeAumSolFromChain(connection: Connection, programId: PublicKey, fundPkStr: string): Promise<number> {
  try {
    ...
    if (rows.length) {
      ...
      const solUsd = solPriceUsd > 0 ? solPriceUsd : 1; // fallback: 1 SOL = 1 SOL
      for (const r of rows) {
        if (r.mint === SOL_MINT) {
          aumSol += r.uiAmount;
          continue;
...
export async function updateDailyPnl() {
  ...
  const lockId = 'pnl-daily-lock';
  const lock = await meta.findOne({ _id: lockId });
  if (lock?.lastRunKey === ymd) {
    console.log('[pnl] Already ran for', ymd);
    return { ok: true, skipped: true };
  }
  await meta.updateOne({ _id: lockId }, { $set: { lastRunKey: ymd, updatedAt: new Date() } }, { upsert: true });
  ...
  const funds = await fundsCol.find(query, { projection: { pnl: 1, fundId: 1, _id: 1, totalDeposits: 1, investments: 1, manager: 1 } }).toArray();
  const yesterday = startOfUtcDay(new Date(today.getTime() - 24 * 3600 * 1000));

  let updated = 0;
  for (const fund of funds) {
    const fid = String((fund as any).fundId || fund._id);
    const aumSol = await computeAumSolFromChain(connection, PROGRAM_ID, fid);
    ...
    const wEntries = Array.isArray(invDoc?.funds?.[fid]) ? invDoc.funds[fid] : [];
    ...
    if (!hasToday) {
      await (fundsCol as any).updateOne({ $or: [{ fundId: fid }, { _id: fid }] }, { $push: { pnl: { date: today, index: indexToday } }, $set: { updatedAt: new Date() } });
      updated++;
    }
  }

  console.log(`[pnl] Daily updater processed=${updated} on ${today.toISOString()}`);
