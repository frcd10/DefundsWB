...
interface RwaWithAccess { fundId: string; name?: string; access?: { type: string; codes?: CodeEntry[]; code?: string }; accessMode?: string }

export function RwaAccessCodesPanel({ product }: { product: RwaWithAccess }) {
  const codes = product.access?.type === 'multi_code' ? (product.access?.codes || []) : [];
  const single = product.access?.type === 'single_code' ? product.access?.code : null;
  ...
  const download = () => {
    ...
    a.href = url; a.download = `rwa_${product.fundId}_codes.txt`; a.click();
    URL.revokeObjectURL(url);
  };

  if (!product.access) return <div className="text-sm text-white/50">No access restrictions.</div>;
  if (product.access.type === 'public') return <div className="text-sm text-white/60">Public product – no codes.</div>;

  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        <h3 className="text-sm font-semibold text-white">Access Codes</h3>
        {product.access.type === 'single_code' && single && (
          <span className="px-2 py-1 rounded bg-white/10 border border-white/15 text-xs font-mono tracking-wider">{single}</span>
        )}
        {product.access.type === 'multi_code' && (
          <>
